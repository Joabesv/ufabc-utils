...
const transformItinerary = (match: string) => {
	...
	const endHour =
		hoursMapping[`${period}${hours[hours.length - 1]}` as HOUR_POS].fim;
...
const formatSchedule = (scheduleString: string) => {
	...
	formattedSchedule = formattedSchedule.replace(
		/\b[2-7][MTN][1-7]{1,2}\b/g,
		transformItinerary,
	);
...
export function handleItinerary(table: HTMLTableElement) {
	walkTree(table);
